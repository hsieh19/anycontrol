...
### ✨ 新增特性 (Added)
- **RS485 从站等待响应超时动态可配置与 NVS 持久化**：
  - 彻底替换 WiFi 主站转发原本硬编码的 200ms 超时，新增 `masterTimeout` 配置项（默认 1000ms，范围 100~3000ms）；
  - 支持通过 NVS 掉电持久化保存并在运行期动态热重载，完美解决现场较慢从站或重载总线下的通信超时与假掉线问题。
- **内置 Web 管理控制台表单补齐 (Port 80)**：
  - 在固件 Web 控制台表单中新增「从站超时 (ms)」输入项（`dmTimeout`），支持本地修改与热重载。
- **REST API 配置响应增强**：
  - `/api/gateway/config` 接口响应体补齐 `masterTimeout` 字段，提升上位机与硬件配置同步的闭环确定性。

### 🐛 稳定性增强 (Security & Stability)
- **总线互斥锁超时上限保护**：
  - 为 `masterTimeout` 增加 `3000ms` 上限约束保护，杜绝异常超长超时导致双主站调度互斥锁长时间阻塞。
